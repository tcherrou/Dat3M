...
{
    char priorvalue = atomic_fetch_sub_explicit(&rw->lock, 1, mo_lock);
    while (priorvalue <= 0) {
...
{
    char priorvalue = atomic_fetch_sub_explicit(&rw->lock, RW_LOCK_BIAS, memory_order_acquire);
...
{
    char priorvalue = atomic_fetch_sub_explicit(&rw->lock, 1, memory_order_acquire);
    if (priorvalue > 0)
        return 1;
