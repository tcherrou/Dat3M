...
{
    while (1) {
        char old_seq = atomic_load_explicit(&l->seq, memory_order_acquire);
        if (old_seq % 2 == 1) {
            continue;
        }

        char res = atomic_load_explicit(&l->data, memory_order_acquire);
...
static inline void write(struct seqlock_s *l, char new_data)
{
    while (1) {
